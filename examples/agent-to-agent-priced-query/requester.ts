/**
 * requester.ts: the agent you run in this example.
 *
 * End-to-end flow:
...
async function main() {
  ...
  const request: PricedQueryRequest = {
    ...
    params: {
      ...
      // No member is read in this flow — see the README banner.
      member_handle: null,
    },
    nonce,
  ...
  const x402 = new X402Client({
    ...
    // This reference implementation demonstrates the secure pattern. The
    // provider always quotes with recipient === env.ALTER_ROUTER_ADDRESS
    // (see provider.ts), so pinning the same address here rejects any
    // envelope a hostile provider might redirect to a different address.
    // Always pass an explicit allowlist: the check is off when omitted.
    recipientAllowlist: [env.ALTER_ROUTER_ADDRESS],
  });
