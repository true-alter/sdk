/**
 * provider.ts: the counterparty agent in this example.
 *
 * Role:
 ...
 *        - return a signed receipt carrying the identity trailers
 *          (`Acted-By: ~yourhandle`, `Drafted-With: ~example`).
 *
  ...
  async fulfil(
    ...
    const stubResponse = {
      ...
      result: {
        ...
        note: 'Stub response. A production compute_belonging scores the caller against a live role archetype vector; this one returns a fixed payload so the flow can be run offline.',
      },
    };

    // The canonical split, with the facilitated routing illustrated.
    // `hasMember: false` is the no-member case: see the README banner
    // and shared.ts:computeSplit for the note it puts on the receipt.
    ...
    const receipt: PricedQueryReceipt = {
      ...
      // Identity trailers. Acted-By names the handle accountable for
      // the call, Drafted-With the instrument that produced it. A
      // Co-Authored-By compatibility trailer is omitted on the wire
      // (it only means anything in a commit message) and would be
      // appended if this receipt were archived into one.
