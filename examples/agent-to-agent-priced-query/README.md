...
~alter's irreducible core #1, **agent-to-agent identity + payment**, per
~alter's internal strategy roadmap. The reference flow is the proof
artefact for the wedge. Two Instrument-tier handles transact across
the ~alter rails with:
...
4. **Signed response**, the provider returns a `PricedQueryReceipt`
   ...
   - a **D-ID8 trailer block** on the receipt itself:
     `Acted-By: ~yourhandle` (Sovereign) and
     `Drafted-With: ~example` (Instrument).
...
|----------|-------------------|
| **D-ID8** | Every signed receipt carries the `Acted-By` / `Drafted-With` trailer block. Requester and provider are Instrument-tier handles; the provider's Sovereign anchor (`Acted-By: ~yourhandle`) is recorded on the wire. |
...
| **D-DS6** | Records the 75 / 5 / 15 / 5 revenue split (member / facilitator / ~alter / cooperative) superseding the original 70/30. Visible in `shared.ts:computeSplit()` and surfaced on every receipt. |
| **`canonical-revenue-split-75-15-5-5-org-is-facilitator-slot`** | Sovereign declaration, 15 June 2026. The split is always 75 / 15 / 5 / 5, with no fifth party and no conditional variant. An org that facilitates a query about someone else occupies the facilitator slot and earns the standard 5%; it never takes a separate slice and never reduces ~alter's 15%. This declaration revoked D-RS8's separate 10% org leg, which the reference flow no longer computes. |
| **D-CO23** | Anti-extraction 5:1 per-stream rule. **The agent-to-agent metadata exclusion is PENDING**, see the banner above. The reference flow's treatment of the member share when no member exists is illustrative-only. |
