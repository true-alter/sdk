...
export interface PricedQueryReceipt {
  ...
  provider: string;
  acted_by: string; // the handle accountable for the call
  drafted_with: string; // the instrument that produced it
  tool: string;
  response: unknown;
  settlement: X402Settlement;
  split: {
    // Bps of the gross amount. A flow with no member has no payee for
    // `member_bps`, and this example leaves that share unallocated.
    member_bps: number;
 ...
 *
 * The split is 75 / 5 / 15 / 5, member / facilitator / ~Alter /
 * cooperative, and the settlement contract enforces the same four
 * constants on chain.
 *
 * This is the ONLY split. There is no fifth party and no conditional
 * variant. A collective that facilitates a query about someone else
 * occupies the FACILITATOR slot and earns the standard 5% facilitator
 * share by virtue of being the facilitator; it never takes a separate
 * slice and never reduces ~Alter's 15%.
 *
 * A flow with no member has no payee for the member share, so the
...
export function computeSplit(input: SplitInput): SplitResult {
  ...
  if (!input.hasMember) {
    notes.push(
      'No member in this flow (agent-to-agent L2 priced query), so the 7500 bps member share ' +
        'has no payee. This example leaves it UNALLOCATED rather than reassigning it; ' +
        'a production flow MUST resolve it before settlement.',
    );
    member = 0;
  }
