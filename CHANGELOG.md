...
- **Write-side bindings the server does not serve** (0.2.4). The SDK
  carried client methods for calls the live MCP server had no handler
  for. They were dropped so the SDK's contract equals the server's,
  and calling one now fails at compile time rather than at run time.
