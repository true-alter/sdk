...
- **`MCPClient` `extraHeaders` option** (0.4.3). Arbitrary HTTP
  headers added to every request, useful when the endpoint sits behind
  an edge gate that needs its own credentials — e.g. Cloudflare Access
  service tokens (`CF-Access-Client-Id` + `CF-Access-Client-Secret`).
  Protocol-level and ALTER-internal headers always win over
  user-supplied collisions.
- **`alter-mcp-bridge` env-var hooks for edge-gate creds** (0.4.3).
  Bridge now reads `CF_ACCESS_CLIENT_ID` + `CF_ACCESS_CLIENT_SECRET`
  (auto-mapped to the corresponding CF-Access headers) and
  `ALTER_BRIDGE_HEADERS` (JSON object, full escape hatch) at start-up.
