/**
 * ES256 per-invocation signing for MCP tool calls.
 *
...
// ---------------------------------------------------------------------------
// Canonical JSON: mirrors ~Alter's server-side per-invocation signature verifier.
// ---------------------------------------------------------------------------
 ...
 *
 * Rules:
 *   - object keys sorted ascending by UTF-16 CODE UNIT, which is what
 *     `Array.prototype.sort` on strings already does, and what RFC 8785
 *     section 3.2.3 requires. NOT by code point: the two orders disagree
 *     whenever a non-BMP key meets one in U+E000..U+FFFF, because a
 *     non-BMP code point encodes to surrogates in U+D800..U+DFFF, which
 *     sort BELOW that range as code units and ABOVE it as code points.
 *   - numbers per ECMAScript `Number::toString`, which is what
 *     `JSON.stringify` emits
 *   - no whitespace
 *   - non-ASCII characters pass through verbatim, not re-encoded as
 *     \\uXXXX. UTF-8 encoding happens at the byte layer before hashing.
 */
...
/**
 * JSON string escape per RFC 8785 section 3.2.2.2: the control set
 * (U+0000..U+001F) with the two-character forms where they exist, the
 * double-quote, and the backslash. Everything else passes through
 * verbatim. `JSON.stringify` does exactly this, so we reuse it.
 *
 * An unpaired surrogate is REFUSED rather than escaped. `JSON.stringify`
 * would happily emit `\ud800` for one, but the string it came from is not
 * well-formed Unicode and has no UTF-8 encoding, so the server that has to
 * hash the same bytes cannot represent it at all. Refusing here turns a
 * server-side signature rejection the caller cannot explain into a local
 * error naming the offending string.
 */
function encodeString(s: string): string {
  if (!isWellFormed(s)) {
    throw new TypeError(
      `canonicalStringify: string contains an unpaired surrogate and has no ` +
        `canonical encoding: ${JSON.stringify(s)}`,
    );
  }
...
/**
 * True when `s` contains no unpaired surrogate.
 *
 * `String.prototype.isWellFormed` is ES2024 and is not assumed: this
 * package supports runtimes older than that, so the scan is done by hand
 * when the built-in is absent.
 */
function isWellFormed(s: string): boolean {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    // A high surrogate must be followed by a low one; a low surrogate
    // reached here was not consumed by a preceding high one.
