...
/**
 * Canonical JSON of the floor document: JCS (RFC 8785), the same form the
 * signer produces. Members sorted by UTF-16 code unit at every depth, no
 * whitespace, non-ASCII passed through as raw UTF-8. The result is the
 * byte-exact input to Ed25519 signing and verification on both sides.
 *
 * Applies every profile bound {@link canonicalStringify} applies, and throws
 * an `AlterCanonicalizationError` for a value outside it: nesting deeper
 * than 64 containers, an integer past 2^53-1 in magnitude, a non-finite
 * number, an unpaired surrogate, or a value JSON cannot represent.
 * Duplicate member names cannot be detected on a value that is already
 * parsed; parse wire text with `parseStrictWire` to refuse them.
 */
