...
// =============================================================================
// Lookups
// Read a tier or a price through these rather than indexing the maps. The
// server resolves an unknown tool name to free at every call site, so a raw
// index that returns undefined disagrees with what the call will actually
// cost.
// =============================================================================

/** Tier for a tool. A name the map does not carry is free, as on the server. */
export function toolTier(toolName: string): number {
  return GENERATED_TOOL_TIERS[toolName] ?? 0;
}

/** Price in USD for one invocation. A tool with no price costs nothing. */
export function toolPrice(toolName: string): number {
  return GENERATED_TOOL_PRICING[toolName] ?? 0;
}
