...
export function atomicJsonMerge(opts: AtomicMergeOptions): WriteResult {
  ...
  if (existsSync(path)) {
    ...
    if (preBytes.trim().length > 0) {
      try {
        // Tolerate a UTF-8 BOM: PowerShell 5.1 Out-File and some Windows
        // editors prepend U+FEFF, which JSON.parse rejects. Found live on
        // Win11 where a BOM'd mcp.json made wiring refuse until hand-fixed.
        // The next atomic write rewrites the file BOM-free.
        ...
        throw new Error(
          `refusing to wire ${path}: existing file is not valid JSON (${(err as Error).message}). ` +
            'Hand-fix the file, then re-run `alter wire`.',
        );
      }
