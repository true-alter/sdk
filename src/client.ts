...
import { checkMinVersion } from './floor-preflight.js';
...
// Public discovery / SDK default: the branded CF-gated host used by external
// agents and discovery manifests. An unauthenticated external agent pointing
// at this URL reaches the correct public surface.
export const DEFAULT_ENDPOINT = 'https://mcp.truealter.com/api/v1/mcp';

// Member bridge runtime endpoint: the bearer-first host used by an
// authenticated member's own bridge or desktop adapter. No CF Access service
// token is required; the member bearer JWT is the only credential. Set
// ALTER_MCP_ENDPOINT (bridge) or ALTER_PUBLIC_MCP_ENDPOINT (mcp-alter) to
// override at runtime.
export const MEMBER_BRIDGE_ENDPOINT = 'https://api.truealter.com/api/v1/mcp';

export const DEFAULT_DOMAIN = 'truealter.com';

// Canonical ~Alter API host used only as the last-resort trust anchor when no
// endpoint is configured and no JWKS override is supplied.
const CANONICAL_API_BASE = 'https://api.truealter.com';
const JWKS_WELL_KNOWN_PATH = '/.well-known/alter-keys.json';

/**
 * Derive the JWKS trust-anchor URL from the configured API surface rather
 * than hardcoding a single host.
 *
 * Resolution order (highest first):
 *   1. `apiBase` option, when supplied.
 *   2. `ALTER_API` environment variable (the SDK's standard API-base env,
 *      matching {@link checkMinVersion}'s `defaultApiBase`).
 *   3. The origin of a configured `endpoint` URL (so a self-hosted /
 *      whitelabel deployment verifies against its own published keys).
 *   4. {@link CANONICAL_API_BASE} as the final fallback.
 *
 * A CNAME-hijack of one bare host can no longer silently become the sole
 * trust anchor: the URL tracks whatever API surface the caller is actually
 * configured to talk to. An explicit `jwksUrl` still overrides this entirely.
 */
export function resolveJwksUrl(opts: {
  jwksUrl?: string;
  apiBase?: string;
  endpoint?: string;
} = {}): string {
  ...
  const base =
    opts.apiBase ??
    (typeof process !== 'undefined' ? process.env?.ALTER_API : undefined) ??
    originOf(opts.endpoint) ??
    CANONICAL_API_BASE;
...
export interface AlterClientOptions extends Omit<MCPClientOptions, 'x402'> {
   ...
   * to discourage casual use: the server-side floor gate still
   * rejects below-floor clients with HTTP 426
...
export class AlterClient {
  ...
  constructor(options: AlterClientOptions = {}) {
    ...
    // Version-floor lazy preflight hook. Fires on the
    // first network call (initialize or any tool call), not on import,
    // not in the constructor. Opt-out via `unsafe_skipVersionCheck`. The
    // hook reuses the `fetch` impl threaded through `options.fetch` so
    // tests + custom HTTP layers see a unified surface.
    const preflightHook = options.unsafe_skipVersionCheck
      ? undefined
      : () =>
          checkMinVersion({
  ...
  // Cross-handle direct messages between authenticated tilde
